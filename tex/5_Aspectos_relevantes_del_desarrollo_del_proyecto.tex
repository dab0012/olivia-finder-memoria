...
Para llevar a cabo el análisis de las redes de dependencias de paquetes de software, contamos con valiosos
datos proporcionados por \textit{libraries.io}\cite{jeremy_katz_2020_3626071} en formato CSV. Estos datos nos permiten extraer la red de
dependencias de los principales repositorios de paquetes de software\footnote{Algunos repositorios
    incluidos en los conjuntos de datos son: npm, Maven, Go, PyPI, NuGet, Packagist, Rubygems, Cargo,
    CocoaPods, Bower, Pub, CPAN, CRAN, Clojars, conda, Hackage, Hex, Meteor, Homebrew, Puppet, Carthage,
    SwiftPM, Julia, Elm, Dub, Racket, Nimble, Haxelib, PureScript, Alcatraz, Inqlude.}. Sin embargo,
es importante destacar que estos datos no están actualizados a la fecha actual debido a la
alta volatilidad de los cambios en los proyectos de software. Por lo tanto, los análisis que
...
Para aprovechar al máximo este conjunto de datos, es importante considerar la temporalidad de la
información y su contexto histórico al realizar análisis y conclusiones. Esto nos permitirá
comprender mejor la dinámica de las redes de dependencias y sus cambios a lo largo del tiempo.
...
que el ecosistema del software evoluciona rápidamente y se introducen constantemente nuevas
versiones y cambios en los paquetes, es crucial contar con información que refleje de manera
precisa el panorama actual de las dependencias entre los paquetes.

\section{API de libraries.io}
...
\begin{itemize}
    \item \textbf{Autenticación}: Para realizar cualquier solicitud a la API, es necesario
          ...
          Esto implica un registro en el servicio\footnote{El registro y obtención de la clave de API
              son necesarios para acceder a las funcionalidades de la API de libraries.io.}.
    \item \textbf{Distintas versiones del servicio}: Existen diferentes versiones del servicio,
          ...
          adicionales están disponibles a través de planes de pago\footnote{La versión gratuita de
              libraries.io tiene ciertas limitaciones y las características avanzadas pueden requerir
    ...
    \item \textbf{Paginación}: Algunas solicitudes pueden devolver múltiples resultados, y para
          ...
          lo cual no es deseable\footnote{El uso de la paginación en las solicitudes a libraries.io
              permite controlar la cantidad de resultados obtenidos por página, pero puede implicar una
              posible pérdida de información en caso de resultados truncados.}.
\end{itemize}
...
\section{Exploración de los repositorios}
...
\section{Análisis empírico de las redes de dependencias de repositorios de paquetes}

\subsection{Análisis de la red de dependencias PyPI}
...
comparación con datos anteriores (en concreto un 474 \% ).\footnote{Este crecimiento refleja el creciente
    interés y contribuciones de la comunidad de desarrolladores a \textit{PyPI}.}
...
\footnote{Es importante destacar que la comunidad de \textit{Python} es conocida por su activa
    participación en la creación y mantenimiento de paquetes en \textit{PyPI}, lo que contribuye al
    enriquecimiento continuo del ecosistema.}
...
\begin{figure}[ht]
    \begin{center}
        ...
        \label{fig:etiqueta}
        \includegraphics[width=0.5\textwidth]{/home/dnllns/Documentos/repositorios/olivia-finder-memoria/img/pypi/circ_common_packages.png}
...
\underline{Grado de salida (out degree)}
...
desarrollo de proyectos y aplicaciones en el entorno de Python. La presencia destacada de estas
bibliotecas en el ranking resalta su importancia dentro del ecosistema de paquetes de Python y
sugiere que son ampliamente adoptadas por los desarrolladores.
...
anterior, lo cual indica su constante relevancia en la comunidad de desarrolladores de Python. Estos
...
Por último, se muestra la distribución de \textit{out degree} para ambos conjuntos de datos.
...
Por otro lado, se ha identificado un incremento en el grado de salida de los paquetes más populares.
...
\begin{figure}[h!]
    \begin{center}
        ...
        \caption{Incremento del out degree en PyPI}
        \label{fig:dependents_increase}
    \end{center}
\end{figure}


\underline{Grado de entrada (In degree)}

Esta métrica nos da una idea del número de dependencias que tiene un paquete dado.
Analizados los datos de libraries.io y representados sobre un grafico obtenemos los siguientes resultados:
...
\begin{figure}[h!]
    \begin{center}
        ...
        \caption{20 paquetes con mayor In degree en libraries.io}
        \label{fig:etiqueta}
    \end{center}
\end{figure}
...
\begin{figure}[h!]
    \begin{center}
        ...
        \caption{20 paquetes con mayor In degree en scraped}
        \label{fig:etiqueta}
    \end{center}
\end{figure}

Si realizamos este mismo análisis con el top 20 de los paquetes con más \textit{in degree} en la actualidad, podemos llegar a
conclusiones similares\footnote{El \textit{in degree} se refiere al número de dependencias que tiene un paquete
    en la red de dependencias}. Se observa una tendencia decreciente en el número de dependencias de los paquetes
...
comunidad de desarrolladores\footnote{La disminución de las dependencias en los paquetes más influyentes puede
    ser resultado de optimizaciones, modularidad mejorada o cambios en las preferencias de los desarrolladores}.
...
\begin{figure}[h!]
    \begin{center}
        ...
        \caption{Distribucion del in degree en libraries.io}
...
\begin{figure}[h!]
    \begin{center}
        ...
        \caption{Distribucion del in degree en scraped}
        \label{fig: Distribucion del in degree en scraped}
    \end{center}
\end{figure}

En el análisis de la distribución de In degree, se ha observado una alta frecuencia de nodos con un bajo grado,
...
Si observamos la evolución, se observa una tendencia similar en ambos casos, aunque en el estado actual se evidencia
...
\begin{figure}[h!]
    \begin{center}
        ...
        \caption{Top 20 pagerank en libraries.io}
...
Si visualizamos el número de dependencias transitivas de estos paquetes, podemos obtener conclusiones más precisas.
...
\begin{figure}[h!]
    \begin{center}
        ...
        \caption{Dependencias transitivas del top 20 pagerank en libraries.io}
...
experimentado una evolución considerable, con una reducción significativa en
...
\begin{figure}[h!]
    \begin{center}
        ...
        \caption{Top 20 pagerank en scraped}
        \label{fig:Top 20 pagerank en scraped}
    \end{center}
\end{figure}
...
PageRank. Además, se observa una disminución general del valor del PageRank en la mayoría de
la red. Esta disminución del PageRank puede interpretarse como una mejora en términos de
vulnerabilidad.
...
\begin{figure}[h!]
    \begin{center}
        \includegraphics[width=1\textwidth]{img/pypi/transitive_scraped.png}
        \caption{Dependencias transitivas del top 20 pagerank en sacraped}
        \label{fig:Dependencias transitivas del top 20 pagerank en sacraped}
    \end{center}
\end{figure}
...
Si invertimos el grafo de nuestra red de dependencias, podemos estudiar el \textit{PageRank}
...
tareas\footnote{Un alto \textit{PageRank} indica la importancia y la confianza que otros paquetes
    depositan en el paquete en cuestión, lo que lo convierte en una fuente valiosa de recursos y
    funcionalidades para otros paquetes en la red.}.
...
él para llevar a cabo sus propias funciones o tareas\footnote{Los paquetes con un alto \textit{PageRank}
    desempeñan un papel fundamental en la red, ya que otros paquetes dependen de ellos para realizar sus
    propias tareas y funciones, lo que los convierte en componentes esenciales de la red de dependencias.}.

Como se puede ver en el top que mostramos a continuación, aparecen los paquetes más conocidos y
comúnmente usados de Python.
...
\begin{figure}[h!]
    \begin{center}
        \includegraphics[width=0.8\textwidth]{img/pypi/librariesio_impact_distribution.png}
        \caption{Distribución del impacto en la red de libraries.io}
        \label{fig:Distribución del impacto en la red de libraries.io}
    \end{center}
\end{figure}

A la vista de la distribución del impacto en la red de libraries.io, se observa un patrón que se
...
no son ampliamente utilizados como dependencias en otros paquetes\footnote{La distribución del impacto
    en la red de libraries.io muestra que hay muchos paquetes con un bajo impacto, lo cual sugiere que no
    son ampliamente utilizados como dependencias en otros paquetes.}.
...
\begin{figure}[h!]
    \begin{center}
        \includegraphics[width=0.8\textwidth]{img/pypi/scraped_impact_distribution.png}
...
Asimismo, se ha identificado otro grupo de tamaño medio-bajo pero que resulta notable debido a
su impacto extremadamente elevado.
...
\begin{flushleft}
    \begin{tabular}{|c|c|c|c|c|c|c|c|}
        \hline
        Size & Head          & Head     & Avg    & Density & Diameter & Clustering  & Transitive   \\
             &               & pagerank & degree &         &          & coeff & deps \\
        \hline
        ...
        6    & omnitools     & 0.436878 & 3.666  & 0.366   & 3        & 0.448       & 4440         \\
